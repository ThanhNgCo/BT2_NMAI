...
class cell{
    ...
    evaluateFValue(destination, source, index){
        // Always use Manhattan heuristic
...
function AStarAlgorithm(){
    ...
    var index = "4"; // Fixed: Manhattan heuristic
...
var selectSD = false; // Disabled: source/destination are set automatically
...
function setup(){
    ...
    source = pickRandomOpenCell();
    do {
        destination = pickRandomOpenCell();
    } while(destination.x === source.x && destination.y === source.y);

    pickedS = true;
    pickedD = true;

    var canvas = createCanvas(grid.width * cellWidth + cellWidth, grid.height * cellHeight + cellHeight);
